...
function set_token(this: Saxo, code: string): Promise<boolean> {
  ...
  return this.fetch(endpoint, url, params)
    .then((res) => res.json())
    .then((token: saxo_t.auth_token_t) => save_token.bind(this)(token))
    .catch(() => false);
}
function refresh_token(this: Saxo, refresh_token: string) {
  const url = conf.saxo.url.auth;
  const params = make_token_params(refresh_token, "refresh_token");
  return this.fetch(endpoint, url, params)
    .then((res) => res.json())
    .then(async (token: saxo_t.auth_token_t) => save_token.bind(this)(token))
    .catch(() => remove_token.bind(this)());
}

async function save_token(this: Saxo, token: saxo_t.auth_token_t) {
