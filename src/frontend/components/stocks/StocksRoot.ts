...
export class StocksRoot extends AppElement {
	...
	private get headers_html() {
		return Brokers.stock_headers.map(name => {
	...
	private get body_children() {
		...
		return stocks.map(stock => {
			const stock_row = this.make_element("stock-row", "", `id="${stock.ticker}"`)
			const positions = this.make_element("positions-root", "", `ticker="${stock.ticker}" class="root-container"`)
