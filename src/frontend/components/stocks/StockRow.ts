...
export class StockRow extends AppElement {
	...
	private add_positions_toggle(row: StockRow) {
		...
		const positions = tbody?.querySelector(`positions-root[ticker="${this.ticker}"]`)
		const button = row.querySelector(`td[name="positions"] button`)!

		//this.parentElement
		//button.setAttribute("onclick", "console.log(this.parentElement)")
		button.addEventListener("click", (_e) => {
			const state = positions?.getAttribute("state") === "open" ? "closed" : "open"
