import type { App, Cache, component_key_t, Events } from ".."

type watch_fnc_t = (old_value: string, new_value: string) => void
type required_string_t<T extends string = string> = T extends "" ? never : T;
...
export class AppElement extends HTMLElement {
	...
	protected make_element = (kind: string, inner_html: string, ...properties: string[]) => {
		return this.parse_html(`<${kind} ${properties.join(" ")}>${inner_html}</${kind}>`)
	}
