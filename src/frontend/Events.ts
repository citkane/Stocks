...
export class Events {
	listen(component: component_t, topic: component_key_t, callback: callback_t) {
		const _callback = (e: Event) => callback((e as unknown as { detail: event_data_t }).detail)

		if (!this.listeners.has(topic)) this.listeners.set(topic, new Set<component_t>());
	...
	dispatch(topic: component_key_t, data: event_data_t) {
		...
		this.listeners.get(topic)?.forEach(listener => listener.dispatchEvent(event))
	}
